...
IN_DIMS = 4
OUT_DIMS = 3 # self attention size
HEAD_DIMS = 8

def self_attention_layer(in_dims:int, out_dims:int, name='self_attention') -> C.Function:
...
def multi_headed_self_attention_layer(num_of_head:int, in_dims:int, seq_dims:int, out_dims:int, sa_dims:int, name='multi_headed_self_attention', as_block:bool = False) -> C.Function:
    ...
    for i in range(num_of_head):
        layers.append(self_attention_layer(IN_DIMS, OUT_DIMS, SA_DIMS, name=name+str(i)))
    outputs = []
    for layer in layers:
...
v = [np.ones(IN_DIMS),np.ones(IN_DIMS),np.ones(IN_DIMS)]
...
# mhsa_layer = multi_headed_self_attention_layer(HEAD_DIMS, IN_DIMS, OUT_DIMS, SA_DIMS, as_block=True)
# model = mhsa_layer(X)
# print(model.eval({model.arguments[0]:v}))
